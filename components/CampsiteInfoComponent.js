...
function RenderCampsite(props) {
    ...
    const panResponder = PanResponder.create({
        ...
        onPanResponderGrant: () => {
            ...
            .then(endState => console.log(endState.finsished ? 'finished' : 'canceled'));
        },
        onPanResponderEnd: (e, gesturedState) => {
            console.log('pan responder end', gesturedState);
            if (recognizeDrag(gesturedState)) {
                Alert.alert(
                    'Add Favorite',
    ...
    const renderCommentItem = ({item}) => {
        return (
            <View style={{margin: 10}}>
                <Text sytle={{frontSize: 14}}>{item.text}</Text>
                <Rating 
